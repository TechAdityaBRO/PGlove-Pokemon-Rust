...
use tauri::{
    ...
    AppHandle, Emitter, LogicalPosition, LogicalSize, State, Webview, WebviewWindow,
};
...
fn start_page_url() -> url::Url {
    url::Url::parse("http://tauri.localhost/start.html").expect("static start page url")
}
...
fn activate_webviews(app: &AppHandle, state: &BrowserState, id: &str) -> Result<(), String> {
    ...
    for tab in &tabs {
        ...
        let Some(wv) = app.get_webview_window(&tab.id) else { continue };
        if active {
            let _ = wv.show();
            let _ = wv.set_focus();
        } else {
...
/// Create the WebView2 webview for a new tab and register it in state.
fn create_tab_webview(app: &AppHandle, url_input: Option<&str>) -> Result<Webview, String> {
    ...
    let builder = WebviewBuilder::new(label.clone(), webview_url_for(url_input.unwrap_or("start")))
        ...
        .on_new_window(move |url, _features| {
            // Route popups / target=_blank links into a brand-new tab.
    ...
    activate_webviews(app, &state, child_wv.label())?;
    Ok(child_wv)
}

/// Create a tab (webview + state entry) and notify the chrome UI.
fn spawn_tab(app: &AppHandle, state: &BrowserState, url: Option<&str>) -> Result<Webview, String> {
    let wv = create_tab_webview(app, url)?;
    emit_state(app, state);
    Ok(wv)
}

// ---------------------------------------------------------------------------
// Commands (all async — required on Windows for webview creation)
// ---------------------------------------------------------------------------
...
pub async fn new_tab(
    ...
    let wv = spawn_tab(&app, &state, url.as_deref())?;
    let id = wv.label().to_string();
    snapshot(&state)
        .into_iter()
        .find(|t| t.id == id)
        .ok_or_else(|| "tab vanished after creation".to_string())
}
...
pub async fn close_tab(
    ...
    if let Some(wv) = app.get_webview_window(&id) {
        let _ = wv.close();
    }
...
pub async fn navigate(
    ...
    let wv = app.get_webview_window(&id).ok_or("no such tab")?;
    match webview_url_for(&url) {
        ...
        WebviewUrl::App(path) => {
            ...
            let u = url::Url::parse(&format!("http://tauri.localhost/{}", path))
                .map_err(|e| e.to_string())?;
            wv.navigate(u).map_err(|e| e.to_string())
        }
        WebviewUrl::CustomProtocol(proto) => {
            update_tab(&app, &id, Some("start".into()), None, Some(true));
            let u = url::Url::parse(&proto).map_err(|e| e.to_string())?;
            wv.navigate(u).map_err(|e| e.to_string())
        }
    }
}
...
pub async fn layout(
    ...
    let tabs = state.tabs.lock().unwrap().clone();
    for tab in &tabs {
        if let Some(wv) = app.get_webview_window(&tab.id) {
...
#[tauri::command]
pub async fn get_state(state: State<'_, BrowserState>) -> Result<Vec<TabInfo>, String> {
    Ok(snapshot(&state))
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Creating webviews needs to happen off the main thread setup path.
